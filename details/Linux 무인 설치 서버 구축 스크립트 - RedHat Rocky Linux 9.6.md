...
1. ISO 이미지 준비 - Rocky Linux 9.6 5.14.0-570.42.2-el9\_6.x86\_64
2. Network: DHCP가 할당 가능한 네트워크(VMware NAT)
3. PXE 서버의 DHCP를 위해 VMware의 DHCP 기능 종료
4. firewalld 설정
...
2. 파일 전송 실패
- 해결: 방화벽 종료로 포트 문제임을 알 수 있음 -> 파일 전송 효율을 위해 프로토콜을 변경, ftp에서 http로 환경 재구축.
3. SCSI 하드디스크 인식 오류
- 해결: 클라이언트의 하드디스크를 SCSI에서 킥스타트 기본 설정과 같은 NVMe로 변경.
4. 설치 시작 부분에서 PXE 서버와 Client의 IP 충돌로 '설치 원천' 오류
- 해결: Kickstart 파일인 'rocky.ks' 파일 내에서 '--bootproto'의 설정인 'static'을 'dhcp'로 변경.
...
* syslinux: 부트 로더 모음
- Why: PXE 환경에서는 BIOS/UEFI가 커널과 initrd를 직접 적재할 수 없어 부트 로더가 대신한다.
- How: 클라이언트는 부트 로더를 다운로드하고 메모리 적재 뒤 실행한다.
* dhcp-server: IP 할당 및 부팅 정보 제공용
- Why: 클라이언트는 파일 수신을 위해 네트워크 주소를 할당 받는다. OS 설치를 위한 부트 로더 파일의 이름을 전달 받는다.
- How: 클라이언트에 IP를 할당하고 부트 로더 파일의 이름을 제공한다.
* tftp-server: 부트 로더 파일 제공용
- Why: PXE ROM이 TCP 스택을 포함하지 않기 때문에 UDP로 부트 로더를 받는다.
- How: 부팅 전 단계에서 TFTP로 부트 로더를 다운로드한다.
* httpd: 설치용 이미지 파일 제공용
- Why: PXE 서버가 OS 설치 이미지 파일을 클라이언트에게 제공한다.
- How: 클라이언트의 커널이 실행된 뒤, 내부 설치 프로그램이 HTTP 프로토콜로 OS 이미지와 설치 파일을 다운로드 한다.
...
* 부트 로더(Boot Loader)란? 부팅에 필요한 커널과 initrd를 적재하기 위한 프로그램이다.
* 커널(Kernel)이란? 운영체제의 핵심으로 CPU, 메모리, 디스크, 네트워크와 같은 하드웨어 자원을 직접 또는 자동으로 제어할 수 있도록 설계된 프로그램이다.
...
* 주요 패키지 설치
root# dnf install -y vim syslinux dhcp-server tftp-server httpd
...
* 이미지 마운트 및 마운트 지점을 PXE 설치 파일 제공용 디렉터리로 변경
...
* https://docs.redhat.com/ko/documentation/red\_hat\_enterprise\_linux/8/html/automatically\_installing\_rhel/index
...
root# chmod 644 /var/www/html/rocky.ks
