...
Kickstart() {
	...
	sed -i "s/^url.+/url --url=http:\/\/$ServerIP\/pub/" "$NewKsFile"
	sed -i "s/^repo.+/ repo --name=\"AppStream\" --baseurl=http:\/\/$ServerIP\/pub\/AppStream/" "$NewKsFile"
	sed -i "s/^network.+/network  --bootproto=dhcp  --hostname=Rocky/" "$NewKsFile"
